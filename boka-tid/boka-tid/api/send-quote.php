...
function buildCustomerHtml(array $data): string {
    ...
    $totals     = $data['totals'] ?? [];  // Mappat från quote-nivå
    $ctx        = $data['context'] ?? [];
    $cust       = $data['customer'] ?? [];
    ...
    if (!empty($materialImage)) {
        $materialImage = absUrl($materialImage);
    }
    ...
    if ($imgUrl) {
        $html .=     '<div style="flex:0 0 90px;"><img src="' . h($imgUrl) . '" alt="' . h($materialName) . '" style="width:90px;height:auto;border-radius:6px;border:1px solid #ddd;object-fit:cover;" /></div>';
    }
    $html .=         '<div style="flex:1;">'
                  .    '<p style="margin:6px 0;"><strong>Material:</strong> ' . h($materialName) . '</p>'
                  .    '<p style="margin:6px 0;"><strong>Form:</strong> ' . h($ctx['shape'] ?? '') . '</p>';
    
    // Visa bänkmått om det finns
    $benchInfo = '';
    if (!empty($ctx['bench']) && is_array($ctx['bench']) && isset($ctx['bench'][0])) {
        $bench = $ctx['bench'][0];
        $benchInfo = h((string)($bench['lengthMm'] ?? '')) . ' × ' . h((string)($bench['depthMm'] ?? '')) . ' mm';
    } elseif (!empty($ctx['uShape'])) {
        $u = $ctx['uShape'];
        $benchInfo = 'U-form: ' . h((string)($u['leftLegMm'] ?? '')) . ' + ' . h((string)($u['centerSpanMm'] ?? '')) . ' + ' . h((string)($u['rightLegMm'] ?? '')) . ' mm';
    }
    
    if ($benchInfo) {
        $html .=       '<p style="margin:6px 0;">' . $benchInfo . '</p>';
    }
...
function buildAdminHtml(array $data): string {
    ...
    // Prisöversikt (ADMIN) – full breakdown
    $html .=   '<h3 style="margin:16px 0 8px 0;color:' . h($brandColor) . ';">Prisöversikt</h3>'
             . '<ul style="margin:0 0 16px 18px;padding:0;color:#333;">'
    ...
    // Visa alla selections
    $sink = $selections['sink'] ?? [];
    $hob = $selections['hob'] ?? [];
    $faucet = $selections['faucet'] ?? [];
    $globalCounts = $data['selections']['globalCounts'] ?? [];
    
    if (!empty($globalCounts['sink']) && $globalCounts['sink'] > 0) {
        $html .= '<li>Diskho: ' . h((string)$globalCounts['sink']) . ' st (montering: ' . h($sink['mount'] ?? 'okänd') . ')</li>';
    }
    if (!empty($globalCounts['hob']) && $globalCounts['hob'] > 0) {
        $html .= '<li>Häll: ' . h((string)$globalCounts['hob']) . ' st (montering: ' . h($hob['mount'] ?? 'okänd') . ')</li>';
    }
    if (!empty($globalCounts['faucet']) && $globalCounts['faucet'] > 0) {
        $html .= '<li>Blandare: ' . h((string)$globalCounts['faucet']) . ' st</li>';
    }
    
    // Visa context info
    if (!empty($ctx['edges'])) {
        $edges = $ctx['edges'];
        $html .= '<li>Kantyp: ' . h($edges['edgeType'] ?? '') . '</li>';
        if (!empty($edges['benchEdgesMm'])) {
            $html .= '<li>Bänkkanter: ' . h((string)$edges['benchEdgesMm']) . ' mm</li>';
        }
        if (!empty($edges['islandEdgesMm'])) {
            $html .= '<li>Ökanter: ' . h((string)$edges['islandEdgesMm']) . ' mm</li>';
        }
    }
    ...
    // Lägg till total yta
    if (!empty($quote['totalStoneM2'])) {
        $html .= '<li>Total yta: ' . number_format((float)$quote['totalStoneM2'], 2, ',', ' ') . ' m²</li>';
    }
...
// Mappa totals från quote-nivå (frontend skickar quote.total, quote.vat)
// Subtotal beräknas som total minus moms
...
$payload['totals'] = [
    ...
    'moms'  => $vat,
    'subtotal' => $subtotal,
    'benchMaterialCost' => 0,  // Dessa finns ej i frontend payload
    'openingsCost' => 0,
    'serviceCost' => 0,
];

// Absolut bild-URL för material
if (!empty($material['image'])) {
    $payload['selections']['material']['image'] = absUrl($material['image']);
}

logQuote('Mapped totals', json_encode($payload['totals']));
