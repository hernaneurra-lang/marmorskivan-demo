...
export default function RendersView({ headers, apiBase }) {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {/* Top devices + cities */}
      {(topDevices.length > 0 || topCities.length > 0) && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
          ...
          {topCities.length > 0 && (
            <div style={{ background: "var(--card-bg)", border: "1px solid var(--border)", borderRadius: 12, padding: 16 }}>
              <div style={{ fontWeight: 600, marginBottom: 10, fontSize: 14 }}>📍 Städer (GPS)</div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: "2px solid var(--border)", textAlign: "left" }}>
                {["Tid", "Material", "Läge", "Enhet", "Plats (GPS)", "Bildstorlek", "Rotation fixad"].map(h => (
            ...
            <tbody>
              {filtered.map(r => (
                <tr key={r.id} style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <td style={{ padding: "9px 12px" }}>
                    {r.city ? <span>{FLAG(r.country_code)} {r.city}, {r.country}</span> : <span style={{ color: "var(--muted)" }}>–</span>}
                  </td>
