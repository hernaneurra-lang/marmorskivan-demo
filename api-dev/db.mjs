...
// One-time patch: remove competitor name references from seam blog post
async function patchBlogSeamPost(db) {
  ...
  const correctedSections = [
    ...
    {
      heading: "Regel nr 1: Placera skarven vid håluttaget",
      content: `<p>Det finaste rådet vi kan ge dig är detta:</p><blockquote><strong>Lägg skarven vid håluttaget – diskho, spishäll eller blandarhål. Där blir den näst intill osynlig.</strong></blockquote><p>Anledningen är enkel. Håluttaget är ett redan befintligt avbrott i stenytan. Ögat förväntar sig att stenen slutar vid diskho-kanten – det registrerar aldrig att det faktiskt är en skarv. Hällen eller hoens ram täcker fogen och gör den bokstavligt talat osynlig uppifrån.</p><p>Det är ett av de viktigaste råden vi ger våra kunder: välj placering av skarvarna redan i planeringsfasen – innan templating – så att håluttagen och skarven hamnar rätt från start.</p>`,
      images: [{ src: "/images/materials/Granit/granit-kitchen.jpg.jpg", alt: "Diskho i granitbänkskiva – idealt ställe för skarv" }]
    },
    {
      heading: "Varför håluttaget fungerar perfekt som skarv-plats",
  ...
  await db.query(
    `UPDATE blog_posts SET sections = $1
     WHERE slug = 'bankskiva-maxlangd-och-skarvar'
     AND sections::text LIKE '%Stenskivor%'`,
